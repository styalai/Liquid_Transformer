...
class LiquidNet(nn.Module):
    ...
    def __init__(self, input_size, num_units):
        ...
        # parameter from _map_inputs
        if (self._input_mapping == MappingType.Affine or self._input_mapping == MappingType.Linear):
            self.w = nn.Parameter(torch.ones(self._input_size))
        if self._input_mapping == MappingType.Affine:
            self.b = nn.Parameter(torch.zeros(self._input_size))

        # init oters parameters
        self._get_variables()
    ...
    def _map_inputs(self, inputs, resuse_scope=False):
        ...
        elif self._input_mapping == MappingType.Affine:
